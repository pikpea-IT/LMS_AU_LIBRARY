...
@push('select2')
  ...
  <link href="{{asset('backend')}}/plugins/select2-bootstrap4-theme/select2-bootstrap4.min.css"
    rel="stylesheet" />
@endpush
...
@section('pagetitle')
  {{trans('Manage Category Blogs')}}
@endsection

@section('content')
@section('breadcrumb_title', trans('List Blog Category'))
@section('breadcrumb_home', trans('Home'))
@section('breadcrumb_pagename', trans('List Blog Category'))

@section('breadcrumb', trans('List Blog Category'))

<div class="row" x-data='{data_record : @json(old())}'>
  {{-- check if can create --}}
    @include('admin.category-blog.crud')
    <div class="col-md-8">
      <div class="card">
        <div class="card-header">
          <h4 class="mb-0 text-primary">
            <i class="bx bxs-user me-1 font-22 text-primary"></i>{{ trans('List Blog Category') }}
          </h4>
        </div>
        <div class="card-body">
          <div class="table-responsive">
            @include('admin.category-blog.list')
          </div>
        </div>
      </div>
    </div>
...
<script>
    function cancel(){
        ...
        $(`#my-form`).find(`[name="slug"]`).val('');

        $(`#status`).prop('checked', false);

        $(`#my-form`).find(`#viewer1`).attr('src', '{{ asset('images/place_holder.png') }}');
    }
    ...
    function edit(data_edit){
        ...
        $(`#my-form`).find(`[name="slug"]`).val(data_edit.slug);
